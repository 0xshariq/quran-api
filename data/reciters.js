export const reciters = [
    {
      "id": 1,
      "name": "Abdullaah 3awwaad Al-Juhaynee",
      "subfolder": "Abdullaah_3awwaad_Al-Juhaynee_128kbps"
    },
    { "id": 2, "name": "Abdullah Basfar", "subfolder": "Abdullah_Basfar_32kbps" },
    { "id": 3, "name": "Abdullah Basfar", "subfolder": "Abdullah_Basfar_64kbps" },
    {
      "id": 4,
      "name": "Abdullah Basfar",
      "subfolder": "Abdullah_Basfar_192kbps"
    },
    {
      "id": 5,
      "name": "Abdullah Matroud",
      "subfolder": "Abdullah_Matroud_128kbps"
    },
    {
      "id": 6,
      "name": "Abdul Basit Mujawwad",
      "subfolder": "Abdul_Basit_Mujawwad_128kbps"
    },
    {
      "id": 7,
      "name": "Abdul Basit Murattal",
      "subfolder": "Abdul_Basit_Murattal_64kbps"
    },
    {
      "id": 8,
      "name": "Abdul Basit Murattal",
      "subfolder": "Abdul_Basit_Murattal_192kbps"
    },
    {
      "id": 9,
      "name": "AbdulSamad",
      "subfolder": "AbdulSamad_64kbps_QuranExplorer.Com"
    },
    {
      "id": 10,
      "name": "Abu Bakr Ash-Shaatree",
      "subfolder": "Abu_Bakr_Ash-Shaatree_64kbps"
    },
    {
      "id": 11,
      "name": "Abu Bakr Ash-Shaatree",
      "subfolder": "Abu_Bakr_Ash-Shaatree_128kbps"
    },
    {
      "id": 12,
      "name": "Ahmed Ibn Ali Al-Ajamy",
      "subfolder": "Ahmed_ibn_Ali_al-Ajamy_64kbps_QuranExplorer.Com"
    },
    {
      "id": 13,
      "name": "Ahmed Ibn Ali Al-Ajamy",
      "subfolder": "Ahmed_ibn_Ali_al-Ajamy_128kbps_ketaballah.net"
    },
    { "id": 14, "name": "Ahmed Neana", "subfolder": "Ahmed_Neana_128kbps" },
    {
      "id": 15,
      "name": "Akram Al Alaqimy",
      "subfolder": "Akram_AlAlaqimy_128kbps"
    },
    {
      "id": 16,
      "name": "Ali Hajjaj Al-Suesy",
      "subfolder": "Ali_Hajjaj_AlSuesy_128kbps"
    },
    { "id": 17, "name": "Ali Jaber", "subfolder": "Ali_Jaber_64kbps" },
    { "id": 18, "name": "Alafasy", "subfolder": "Alafasy_64kbps" },
    { "id": 19, "name": "Alafasy", "subfolder": "Alafasy_128kbps" },
    { "id": 20, "name": "Ayman Sowaid", "subfolder": "Ayman_Sowaid_64kbps" },
    { "id": 21, "name": "Aziz Alili", "subfolder": "aziz_alili_128kbps" },
    { "id": 23, "name": "Fares Abbad", "subfolder": "Fares_Abbad_64kbps" },
    { "id": 24, "name": "Ghamadi", "subfolder": "Ghamadi_40kbps" },
    { "id": 25, "name": "Hani Rifai", "subfolder": "Hani_Rifai_64kbps" },
    { "id": 26, "name": "Hani Rifai", "subfolder": "Hani_Rifai_192kbps" },
    { "id": 27, "name": "Hudhaify", "subfolder": "Hudhaify_32kbps" },
    { "id": 28, "name": "Hudhaify", "subfolder": "Hudhaify_64kbps" },
    { "id": 29, "name": "Hudhaify", "subfolder": "Hudhaify_128kbps" },
    { "id": 30, "name": "Husary", "subfolder": "Husary_64kbps" },
    { "id": 31, "name": "Husary", "subfolder": "Husary_128kbps" },
    {
      "id": 32,
      "name": "Husary Mujawwad",
      "subfolder": "Husary_Mujawwad_64kbps"
    },
    {
      "id": 33,
      "name": "Husary Mujawwad",
      "subfolder": "Husary_128kbps_Mujawwad"
    },
    {
      "id": 34,
      "name": "Husary (Muallim)",
      "subfolder": "Husary_Muallim_128kbps"
    },
    { "id": 35, "name": "Ibrahim Akhdar", "subfolder": "Ibrahim_Akhdar_32kbps" },
    { "id": 36, "name": "Ibrahim Akhdar", "subfolder": "Ibrahim_Akhdar_64kbps" },
    {
      "id": 37,
      "name": "Khalefa Al-Tunaiji",
      "subfolder": "khalefa_al_tunaiji_64kbps"
    },
    {
      "id": 38,
      "name": "Khalid Abdullah al-Qahtanee",
      "subfolder": "Khaalid_Abdullaah_al-Qahtaanee_64kbps"
    },
    {
      "id": 39,
      "name": "Khalid Abdullah al-Qahtanee",
      "subfolder": "Khaalid_Abdullaah_al-Qahtaanee_128kbps"
    },
    { "id": 40, "name": "Karim Mansoori", "subfolder": "Karim_Mansoori_40kbps" },
    {
      "id": 41,
      "name": "Maher Al Muaiqly",
      "subfolder": "Maher_AlMuaiqly_64kbps"
    },
    {
      "id": 42,
      "name": "Maher Al Muaiqly",
      "subfolder": "MaherAlMuaiqly128kbps"
    },
    {
      "id": 43,
      "name": "Mahmoud Ali Al-Banna",
      "subfolder": "mahmoud_ali_al_banna_32kbps"
    },
    { "id": 44, "name": "Menshawi", "subfolder": "Menshawi_16kbps" },
    { "id": 45, "name": "Menshawi", "subfolder": "Menshawi_32kbps" },
    {
      "id": 46,
      "name": "Minshawy Mujawwad",
      "subfolder": "Minshawy_Mujawwad_64kbps"
    },
    {
      "id": 47,
      "name": "Minshawy Mujawwad",
      "subfolder": "Minshawy_Mujawwad_192kbps"
    },
    {
      "id": 48,
      "name": "Minshawy Murattal",
      "subfolder": "Minshawy_Murattal_128kbps"
    },
    {
      "id": 49,
      "name": "Mohammad al Tablaway",
      "subfolder": "Mohammad_al_Tablaway_64kbps"
    },
    {
      "id": 50,
      "name": "Mohammad al Tablaway",
      "subfolder": "Mohammad_al_Tablaway_128kbps"
    },
    {
      "id": 51,
      "name": "Muhammad AbdulKareem",
      "subfolder": "Muhammad_AbdulKareem_128kbps"
    },
    {
      "id": 52,
      "name": "Muhammad Ayyoub",
      "subfolder": "Muhammad_Ayyoub_32kbps"
    },
    {
      "id": 53,
      "name": "Muhammad Ayyoub",
      "subfolder": "Muhammad_Ayyoub_64kbps"
    },
    {
      "id": 54,
      "name": "Muhammad Ayyoub",
      "subfolder": "Muhammad_Ayyoub_128kbps"
    },
    {
      "id": 55,
      "name": "Muhammad Jibreel",
      "subfolder": "Muhammad_Jibreel_64kbps"
    },
    {
      "id": 56,
      "name": "Muhammad Jibreel",
      "subfolder": "Muhammad_Jibreel_128kbps"
    },
    {
      "id": 57,
      "name": "Muhsin Al Qasim",
      "subfolder": "Muhsin_Al_Qasim_64kbps"
    },
    {
      "id": 58,
      "name": "Muhsin Al Qasim",
      "subfolder": "Muhsin_Al_Qasim_128kbps"
    },
    {
      "id": 59,
      "name": "Nasser Al Qatami",
      "subfolder": "Nasser_Alqatami_128kbps"
    },
    { "id": 60, "name": "Parhizgar", "subfolder": "Parhizgar_64kbps" },
    {
      "id": 61,
      "name": "Saber AbdulHakam",
      "subfolder": "Saber_AbdulHakam_64kbps"
    },
    {
      "id": 62,
      "name": "Saood Ash-Shuraym",
      "subfolder": "Saood_ash-Shuraym_64kbps"
    },
    {
      "id": 63,
      "name": "Saood Ash-Shuraym",
      "subfolder": "Saood_ash-Shuraym_192kbps"
    },
    { "id": 64, "name": "Sudais", "subfolder": "Sudais_64kbps" },
    { "id": 65, "name": "Sudais", "subfolder": "Sudais_128kbps" },
    {
      "id": 66,
      "name": "Tawfeeq As-Sayegh",
      "subfolder": "Tawfeeq_As-Sayegh_64kbps"
    }
  ]
  